...
const titleScreenWindow: HTMLElement = document.querySelector(
    "main.title-screen"
) as HTMLElement;
const questionWindow: HTMLElement = document.querySelector(
    "main.question-window"
) as HTMLElement;
const finalResultsWindow: HTMLElement = document.querySelector(
    "main.final-results"
) as HTMLElement;

const turnIndicatorElement: HTMLDivElement = document.querySelector(
...
const playerScoreValueElement: HTMLSpanElement = document.querySelector(
    "main.question-window > section.score > div.player-score > span.player-score-value"
) as HTMLSpanElement;
const sallyScoreValueElement: HTMLSpanElement = document.querySelector(
    "main.question-window > section.score > div.sally-score > span.sally-score-value"
) as HTMLSpanElement;

const winnerElement: HTMLElement = document.querySelector(
    "main.final-results > section.winner"
);
const playerFinalScoreValueElement: HTMLSpanElement = document.querySelector(
    "main.final-results > section.scores > div.player-score > span.player-score-value"
) as HTMLSpanElement;
const sallyFinalScoreValueElement: HTMLSpanElement = document.querySelector(
    "main.final-results > section.scores > div.sally-score > span.sally-score-value"
) as HTMLSpanElement;
...
async function gameMainLoop(): Promise<void> {
    ...
    if (questions.length === 0) {
        try {
            questions = (await fetch(`/questions/${questionsId}.json`).then(
                (res) => res.json()
            )) as Array<Question>;
    ...
    for (let question of questions) {
        ...
        let answerChoice: number = await (async function (): Promise<number> {
            return new Promise((resolve) => {
                let choicesChildren: Array<HTMLButtonElement> = Array.from(
        ...
        let choicesElementChildren: Array<HTMLButtonElement> = Array.from(
            choicesElement.childNodes
        ) as Array<HTMLButtonElement>;

        choicesElementChildren[answerChoice].classList.add("wrong-answer");

        choicesElementChildren[question.correctAnswer].classList.remove(
            "wrong-answer"
        );

        choicesElementChildren[question.correctAnswer].classList.add(
            "correct-answer"
        );
        ...
        let sallyAnswerChoice = sampleFromProbabilityList(
...
choicesElement.addEventListener("click", () => {
    for (let choiceElement of Array.from(
        choicesElement.childNodes
